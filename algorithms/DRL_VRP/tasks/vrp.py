...
class VehicleRoutingDataset(Dataset):
    ...
    def update_mask(self, mask, dynamic, chosen_idx=None):
        ...
        if repeat_home.any():
            new_mask[torch.nonzero(repeat_home), 0] = 1.
        if (~repeat_home).any():
            new_mask[torch.nonzero(~repeat_home), 0] = 0.
        ...
        if combined.any():
            new_mask[torch.nonzero(combined), 0] = 1.
            new_mask[torch.nonzero(combined), 1:] = 0.
    ...
    def update_dynamic(self, dynamic, chosen_idx):
        ...
        if visit.any():
            ...
            visit_idx = torch.nonzero(visit).squeeze()
        ...
        if depot.any():
            all_loads[torch.nonzero(depot).squeeze()] = 1.
            all_demands[torch.nonzero(depot).squeeze(), 0] = 0.

        tensor = torch.cat((all_loads.unsqueeze(1), all_demands.unsqueeze(1)), 1)
        return tensor.to(dynamic.device)
